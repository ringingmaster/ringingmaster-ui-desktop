...
public class TouchPersistence {
	...
	public void save(Path path, ObservableTouch touch) {
	...
	public ObservableTouch load(Path path) {
		TouchPersist touchPersist = documentPersist.readTouch(path);
		ObservableTouch touch = buildTouch(touchPersist);
		return touch;
	}

	TouchPersist buildTouchPersist(ObservableTouch touch) {
		//TODO
		return null;
...
//		return touchPersist;
	...
	@VisibleForTesting
	protected ObservableTouch buildTouch(TouchPersist touchPersist) {
//TODO should we store this?		int rowCount = touchPersist.getCells().getRows();
//TODO should we store this?//		int columnCount = touchPersist.getCells().getColumns();
		NumberOfBells numberOfBells = NumberOfBells.valueOf(touchPersist.getNumberOfBells());
		ObservableTouch touch = TouchBuilder.newTouch(numberOfBells);
		...
		if (touchPersist.getNonSplicedActiveNotation() != null) {
			...
			Set<NotationBody> allNotations = touch.get().getAllNotations();
		...
		touch.setStartChange(MethodBuilder.parse(touch.get().getNumberOfBells(), touchPersist.getStartChange()));
		...
		if (touchPersist.getTerminationChange() != null) {
			touch.setTerminationChange(MethodBuilder.parse(touch.get().getNumberOfBells(), touchPersist.getTerminationChange()));
		}
